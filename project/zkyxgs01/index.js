...
/***/ function(module, exports, __webpack_require__) {
	...
	function UXPerson(o){
		...
		this.type = o.type;
		this.item = o.item;
		this.word = GLOBAL_DATA.renderData.children[this.type].children[this.item].word;
		this.link = GLOBAL_DATA.renderData.children[this.type].children[this.item].url;
		this.name = GLOBAL_DATA.renderData.children[this.type].children[this.item].name;
		this.TopOrBottom = Math.floor(Math.random()*2)==0?'bottom':'top';
		this.height = Math.random()*80;
		this.left = (100 + Math.random() * (GLOBAL_DATA.SCREEN_WIDTH -500)).toFixed(0);
		this.index = o.index;
		this.eyeBlinkDelay = (Math.random()*1).toFixed(2);
		this.eleblinkAnimationIndex = 1+Math.floor(Math.random()*2);

	}
	UXPerson.prototype.render = function(){
		var _person = d3.select(this.renderer).append('div');
		_person
		.attr('class',function(){
			if(this.TopOrBottom == 'top') return 'uxperson top';
			return 'uxperson bottom';
		}.bind(this))
		.attr('title',this.name)
		.attr('style',function(){
			var _argColor1 = this.colorCube[this.type][0];
			var _argColor2 = this.colorCube[this.type][1];
			if(this.TopOrBottom == 'top'){
				var _argColor1 = this.colorCube[this.type][1];
				var _argColor2 = this.colorCube[this.type][0];
			}
			return 'height:0px;background:linear-gradient(to bottom, '+_argColor1+', '+_argColor2+');left:'+this.left+'px;'+this.TopOrBottom+':-25px';
		}.bind(this))
		.transition()
		.delay(function(){
		...
		.attr('style',function(){
			...
			return 'height:'+(110+this.height)+'px;background:linear-gradient(to bottom, '+_argColor1+', '+_argColor2+');left:'+this.left+'px;'+this.TopOrBottom+':-25px; animation:uxperson-body-'+this.TopOrBottom+' 1s infinite';
		}.bind(this));
		...
		var _leftEye = _person.append('div')
		.attr('class','left-eye')
		.attr('style',function(){
			return 'animation:uxperson-eye-blink-'+this.eleblinkAnimationIndex+' 5s '+this.eyeBlinkDelay+'s infinite;'
		}.bind(this));
		var _rightEye = _person.append('div')
		.attr('class','right-eye')
		.attr('style',function(){
			return 'animation:uxperson-eye-blink-'+this.eleblinkAnimationIndex+' 5s '+this.eyeBlinkDelay+'s infinite;'
		}.bind(this));

		var _wordWrap = _person.append('div')
		.attr('class','icon-wrap')
		.text(this.word);
	}
	// --------------------------------------------------
	GLOBAL_DATA.imgBaseUrl = './';
	d3.json('./tree.json',function(error,root){
		if(error) throw error;
		GLOBAL_DATA.renderData = root;
		// 首次加载导航页
		loadNavPage();
		// 检查URL查询字符设置相应效果
		checkInnerUrl();
		...
		// ----------------------------------------------------------
		// 小怪物
		var UXPersonwRenderer = document.getElementById('ux_home_renderer');
		if(localStorage.getItem('ApplicationUXPerson')){
			var ApplicationUXPerson = JSON.parse(localStorage.getItem('ApplicationUXPerson'));
			for(var i = 0; i<ApplicationUXPerson.length; i++){
				if(ApplicationUXPerson[i].type){
					var uxperson = new UXPerson({
						renderer: UXPersonwRenderer,
						type: ApplicationUXPerson[i].type,
						item: ApplicationUXPerson[i].item,
						index: i
					});
					uxperson.render();
				}
	...
	var getType = function(){
		var search = window.location.href.split('#')[1].split('&')[0].slice(2);
		return search;
	};
	var getIndex = function(){
		var search = window.location.href.split('#')[1].split('&')[1].slice(5);
		return search;
	};
	var setUsuallVisit = function(_type, _item){
		var max = 8;
		var ApplicationUXPerson = localStorage.getItem('ApplicationUXPerson');
		if(ApplicationUXPerson){
			ApplicationUXPerson = JSON.parse(ApplicationUXPerson);
		}else{
			ApplicationUXPerson = [];
		}
		ApplicationUXPerson.unshift({
			"type": _type,
			"item": _item
		});
		if(ApplicationUXPerson.length > max){
			ApplicationUXPerson.length = max;
		}
		ApplicationUXPerson = JSON.stringify(ApplicationUXPerson);
		localStorage.setItem('ApplicationUXPerson',ApplicationUXPerson);

	}
	function checkInnerUrl(){
		var search = window.location.href.split('#')[1];
		// 存在#查询参数
		if(search && search.length > 0){
			var n = search.split('&')[0].slice(2);
			var item = Number(search.split('&')[1].slice(5));
			switch(n){
				case 'system-design':
					switchChildSystem('design',0,'#b83f64',item);
					break;
				case 'system-example':
					switchChildSystem('example',1,'#3aae97',item);
					break;
				case 'architecture':
					switchChildSystem('architecture',2,'#b35ed5',item);
					break;
				default:
					break;
			}
		}else if(!search || search && search.length <= 0){
			d3.select('#page_system_design')
		  	.style({'left':'100%'});
			d3.select('#page_system_example')
		  	.style({'left':'100%'});
			d3.select('#page_system_architecture')
		  	.style({'left':'100%'});
		}
	};
	// 内参、帮助、我们导航
	(function(){
		var pageTopMoreInfo = d3.select('.page-top-more-info');

		pageTopMoreInfo.selectAll('.first')
		.on('mouseover',function(){
			var _wrapId = d3.select(this).attr('data-list-id');
			d3.select('#'+_wrapId)
			.attr('style',function(){
		...
		.on('click',function(){
			window.location.href = d3.select(this).attr('data-link');
		});
	}());
	// ---------------------------导航页----事件注册--------------------------------------
	// 主导航页系统设计点击事件
	d3.select('.page-top-nav-system-design-wrap').on('click',function(){
		clickNavButton({
			type: 'design',
			index: 0,
			point_fill: '#b83f64'
	...
	// 主导航页系统实例点击事件
	d3.select('.page-top-nav-system-example-wrap').on('click',function(){
		clickNavButton({
			type: 'example',
			index: 1,
			point_fill: '#3aae97'
	...
	// 主导航页系统应用点击事件
	d3.select('.page-top-nav-system-architecture-wrap').on('click',function(){
	  	clickNavButton({
			type: 'architecture',
			index: 2,
			point_fill: '#b35ed5'
	...
	// ---------------------------面包屑导航栏--事件注册----------------------------------------
	d3.selectAll('.go-nav-page').on('click',function(){
		d3.selectAll('.page.page-item')
		.style({
			'left':'100%'
		});
	},false);
	...
	// 切换子系统页
	function switchChildSystem(_type,_index,_circleFill,_item){
		d3.selectAll('page page-system')
		.style({'left':'100%'});
		// 当前系统显示
		d3.select('#page_system_'+_type)
	  	.style({'left':'0%'});

	  	d3.select('#'+_type+'_nav_list_wrap').selectAll('circle')
	  	.each(function(d,i){
	  		if(i == _item){
				d3.select(this)
	  		...
	  		d3.select(this).attr('style','animation: none;');
			setTimeout(function(){
				d3.select(this).attr('style','animation: circle-animation 0.8s 0.'+i+'s ease-out;');
			}.bind(this),10);
	  	});
	  	// 设置中文标题
		d3.select('#'+_type+'_name_ch').style('animation','title-out-animation .2s forwards');
		setTimeout(function(){
			d3.select('#'+_type+'_name_ch')
			.text(GLOBAL_DATA.renderData.children[_index].children[_item].name)
			.style('animation','title-in-animation .6s forwards');
		},300);
		// 设置英文标题
		d3.select('#'+_type+'_name_en').style('animation','title-out-animation .2s .1s forwards');
		setTimeout(function(){
			d3.select('#'+_type+'_name_en')
			.text(GLOBAL_DATA.renderData.children[_index].children[_item].name_en)
			.style('animation','title-in-animation .6s forwards');
		},500);
		// 设置图片链接
		d3.select('#'+_type+'_media_link')
		.attr('target','_blank')
		.attr('href',GLOBAL_DATA.renderData.children[_index].children[_item].url);
		// 设置图片
		d3.select('#'+_type+'_media')
		.attr('src',GLOBAL_DATA.imgBaseUrl+'system-'+_type+'/'+GLOBAL_DATA.renderData.children[_index].children[_item].img);
	}
	// 绘制导航按钮
	function drawNavButton(_type, _index, _Fill){
		var buttonNums = GLOBAL_DATA.renderData.children[_index].children.length;
		// 左侧
		var _buttonWrapLeft = d3.select('#'+_type+'_nav_list_left').selectAll('a')
		.data(GLOBAL_DATA.renderData.children[_index].children)
		.enter()
		.append('a')
		...
		.attr('target','_blank')
		.attr('data-parent-type',function(){
			switch(_type){
				case 'design':
					return 0;
					break;
		...
		.attr('style', function(d,i){
			var _top = d3.select(this).attr('data-index');
			return 'top:'+_top*55+'px; background-color:'+_Fill;
		})
		.on('mouseover',function(d,i){
			d3.select(this)
			.attr('class','nav-list-item active')
			.attr('style', function(d,i){
				var _top = d3.select(this).attr('data-index');
				return 'top:'+_top*55+'px; background-color:#f4c021';
			})
			...
			// 设置图片
			d3.select('#'+_type+'_media')
			.attr('src',GLOBAL_DATA.imgBaseUrl+'system-'+_type+'/'+d.img);
			// 设置中文标题
			d3.select('#'+_type+'_name_ch').style('animation','title-out-animation .2s forwards');
			setTimeout(function(){
				d3.select('#'+_type+'_name_ch')
		...
		// 右侧
		var _buttonWrapRight = d3.select('#'+_type+'_nav_list_right').selectAll('a')
		.data(GLOBAL_DATA.renderData.children[_index].children)
		.enter()
		.append('a')
		.attr('href',function(d,i){
		...
		.attr('style', function(d,i){
			var _top = d3.select(this).attr('data-index')-9;
			return 'top:'+_top*55+'px; background-color:'+_Fill;
		})
		.on('mouseover',function(d,i){
			d3.select(this)
		...
		// 底部
		var _buttonWrapBottom = d3.select('#'+_type+'_nav_list_bottom').selectAll('a')
		.data(GLOBAL_DATA.renderData.children[_index].children)
		.enter()
		.append('a')
		.attr('href',function(d,i){
		...
		.attr('style', function(d,i){
			var _left = d3.select(this).attr('data-index')-18;
			return 'left:'+_left*54+'px; background-color:'+_Fill;
		})
		.on('mouseover',function(d,i){
			d3.select(this)
			.attr('class','nav-list-item active')
			.attr('style', function(d,i){
				var _left = d3.select(this).attr('data-index')-18;
				return 'left:'+_left*54+'px; background-color:#f4c021';
			})
		...
		// 通用点击事件
		d3.selectAll('.nav-list-item')
		.on('click',function(d,i){
			setUsuallVisit(d3.select(this).attr('data-parent-type'), d3.select(this).attr('data-index'));
		})
	}
	// ----------------------------初次加载导航页------------------------------
	function loadNavPage(){
		// 设置各系统名称
		d3.select('.page-top-title-name')
		.text(GLOBAL_DATA.renderData.name);

		d3.select('#nav_design_name_ch')
		.html(GLOBAL_DATA.renderData.children[0].name);
		d3.select('#nav_design_name_en')
		.html(GLOBAL_DATA.renderData.children[0].name_en);

		d3.select('#nav_example_name_ch')
		.text(GLOBAL_DATA.renderData.children[1].name);
		d3.select('#nav_example_name_en')
		.text(GLOBAL_DATA.renderData.children[1].name_en);

		d3.select('#nav_architecture_name_ch')
		.text(GLOBAL_DATA.renderData.children[2].name);
		d3.select('#nav_architecture_name_en')
		.text(GLOBAL_DATA.renderData.children[2].name_en);

		// 设置系统设计导航
		drawNavButton('design',0,'#e10659');
		// 设置系统实例导航
		drawNavButton('example',1,'#09d2b6');
		// 设置系统架构导航
		drawNavButton('architecture',2,'#a20edf');
	}
	// -------------------------点击导航页系统按钮事件------------------------
	function clickNavButton(o){
		var _type = o.type;
		var _index = o.index;
		var _pointFill = o.point_fill;

		// 显示页面
	  	d3.select('#page_system_'+_type)
	  	.style({'left':'0%'});
		// 设置左侧圆点按钮
	  	d3.select('#'+_type+'_nav_list_wrap')
		.selectAll('circle')
		.transition()
		.each(function(d,i){
			if(i == 0){
				d3.select(this).attr('stroke','#3aae97').transition()
				.attr('fill', _pointFill)
				.attr('stroke','#f4de21')
				.attr('stroke-width','2');
			}else{
				d3.select(this)
			...
			setTimeout(function(){
				d3.select(this)
				.attr('style','animation: circle-animation 0.8s 0.'+i+'s ease-out;');
			}.bind(this),10);
		});
		// 设置图片链接
		d3.select('#'+_type+'_media_link')
		.attr('target','_blank')
		.attr('href',GLOBAL_DATA.renderData.children[_index].children[0].url);
		// 设置图片
		d3.select('#'+_type+'_media')
		.style('cursor','pointer')
		.attr('src',GLOBAL_DATA.imgBaseUrl+'system-'+_type+'/'+GLOBAL_DATA.renderData.children[_index].children[0].img);
		// 设置中文标题
		d3.select('#'+_type+'_name_ch').style('animation','title-out-animation .2s forwards');
		setTimeout(function(){
			d3.select('#'+_type+'_name_ch')
			.text(GLOBAL_DATA.renderData.children[_index].children[0].name)
			.style('animation','title-in-animation .6s forwards');
		},300);
		// 设置英文标题
		d3.select('#'+_type+'_name_en').style('animation','title-out-animation .2s .1s forwards');
		setTimeout(function(){
			d3.select('#'+_type+'_name_en')
			.text(GLOBAL_DATA.renderData.children[_index].children[0].name_en)
			.style('animation','title-in-animation .6s forwards');
		},500);
	}
	// 星空背景
	GLOBAL_DATA.SCREEN_WIDTH = window.screen.width;
	GLOBAL_DATA.SCREEN_HEIGHT = window.screen.height;
	GLOBAL_DATA.RATIO = 1;
	var nav_bg_canvas = document.querySelector('#nav_bg_canvas');
	nav_bg_canvas.style.opacity = '0.6';
	var ctx_nav_bg = nav_bg_canvas.getContext('2d');
	nav_bg_canvas.width = GLOBAL_DATA.SCREEN_WIDTH * GLOBAL_DATA.RATIO;
	nav_bg_canvas.height = GLOBAL_DATA.SCREEN_HEIGHT * GLOBAL_DATA.RATIO;
	nav_bg_canvas.style.width = GLOBAL_DATA.SCREEN_WIDTH+'px';
	nav_bg_canvas.style.height = GLOBAL_DATA.SCREEN_HEIGHT+'px';
	GLOBAL_DATA.navBgCanvas = nav_bg_canvas;
	GLOBAL_DATA.ctxNavBg = ctx_nav_bg;
	var colorCube = [
		['#b83f64','#c5ad4d'],
		['#3aae97','#4252b9'],
		['#b35ed5','#4580cf']
	...
	// 类
	function Star(radius, color, alpha){
		this.ratio = window.devicePixelRatio;
		if(radius === undefined){radius = 40;}
		if(color === undefined){color = '#444';}
		this.x = 0;
		this.y = 0;
		this.radius = radius;
		this.rotation = 0;
		this.scaleX = 1;
		this.scaleY = 1;
		var temp_random = Math.round(Math.random()*(colorCube.length-1));
		var gradient = ctx_nav_bg.createLinearGradient(-this.radius,-this.radius,this.radius,this.radius);
		gradient.addColorStop(0,colorCube[temp_random][0]);
		gradient.addColorStop(1,colorCube[temp_random][1]);
		this.color = gradient;
		this.lineWidth = 0;
		this.alpha = alpha || 0.2+Math.random()
	}
	Star.prototype.draw = function(context){
		...
		context.beginPath();
		context.arc(0, 0, this.radius*this.ratio, 0, (Math.PI*2), true);
		context.closePath();
		context.fill();
		if(this.lineWidth > 0){
			context.stroke();
	...
	// -------------------------------------------
	particles = [],
	numParticles = GLOBAL_DATA.SCREEN_WIDTH*0.015,
	minDist = 210,
	minNum = numParticles,
	springAmount = 0.00001;
	for(var size, particle, i=0; i<numParticles; i++){
		size = Math.random() * 11 + 3;
		particle = new Star(size,colorCube[Math.round(Math.random()*colorCube.length)],(0.5+Math.random()));
		particle.x = Math.random() * GLOBAL_DATA.SCREEN_WIDTH;
		particle.y = Math.random() * GLOBAL_DATA.SCREEN_HEIGHT;
	...
	function draw(particle){
		particle.draw(ctx_nav_bg);
	}
	function scaleAlpha(dist){
	...
	function spring(partA, partB){
		...
		if(dist < minDist){
			ctx_nav_bg.save();
			ctx_nav_bg.globalAlpha = scaleAlpha(dist);
			ctx_nav_bg.strokeStyle = '#ffffff';
			ctx_nav_bg.beginPath();
			ctx_nav_bg.moveTo(partA.x, partA.y);
			ctx_nav_bg.lineTo(partB.x, partB.y);
			ctx_nav_bg.closePath();
			ctx_nav_bg.stroke();
			ctx_nav_bg.restore();
			...
			partB.vx -= ax / partA.mass;
			partB.vy -= ay / partA.mass;
		}
	}
	function move(partA, i){
		partA.x += partA.vx;
		partA.y += partA.vy;
		if(partA.x > (GLOBAL_DATA.SCREEN_WIDTH+partA.radius + minDist)){
			if(particles.length > minNum){
				particles.shift();
			}else{
				partA.x = -partA.radius;
			}
		}else if(partA.x < (-partA.radius)){
			partA.x = GLOBAL_DATA.SCREEN_WIDTH+partA.radiu;
		}
		if(partA.y > (GLOBAL_DATA.SCREEN_HEIGHT+partA.radius + minDist)){
			partA.y = 0;
		}else if(partA.y < 0){
			partA.y = GLOBAL_DATA.SCREEN_HEIGHT;
		}
		for(var partB, j=i+1; j<particles.length; j++){
			partB = particles[j];
			spring(partA, partB);
		}
	}
	...
	(function drawFrame(){
		window.requestAnimationFrame(drawFrame, nav_bg_canvas);
		ctx_nav_bg.clearRect(0, 0, GLOBAL_DATA.SCREEN_WIDTH, GLOBAL_DATA.SCREEN_HEIGHT);
	...
	function UXElement(){
		// 设置颜色等
		GLOBAL_DATA.innerWidth = window.innerWidth;
		GLOBAL_DATA.innerHeight = window.innerHeight;
	}
	UXElement.prototype.init = function(_renderer){
		this.renderWrapId = _renderer;
		this.renderer = d3.selectAll('.'+this.renderWrapId);
		this.refresh({
			width: GLOBAL_DATA.innerWidth,
			height: GLOBAL_DATA.innerHeight
		});
	}
	UXElement.prototype.refresh = function(o){
		// 左上角
		var _top_left_group = d3.selectAll('.top-left-group');
		_top_left_group
		.attr('transform',function(){
			var _x = - 20;
			var _y = - 40;
			return 'translate('+_x+', '+_y+') rotate(40)'
		});
		// 右下角
		var _bottom_right_group = d3.selectAll('.bottom-right-group');
		_bottom_right_group
		.attr('transform',function(){
			var _x = o.width - 190;
			var _y = o.height - 260;
	...
	// ----------------------------------------------------------------------
	var uxelement = new UXElement();
	uxelement.init('ux_element_wrap');
	// 缩放监听
	...
	// ---------------------------------------------------
	d3.selectAll('.go-nav-page')
	.on('click',function(){
		window.location.href = "#";
		checkInnerUrl();

		d3.selectAll('.uxperson').remove();
